...
public struct Utils {
    ...
    public static func multicurve(points:[XYProvidable], context:CGContext) {
        var p0:XYProvidable!
        var p1:XYProvidable!
        var midx:Float!
        var midy:Float!
        ...
        while i < points.count - 2 {
            ...
            midx = (p0.x + p1.x) / 2
            midy = (p0.y + p1.y) / 2
            context.addQuadCurve(to: CGPoint(x: midx, y: midy), control: CGPoint(x: p0.x, y: p0.y))
            i += 1
        }
        
        p0 = points[points.count - 2]
        p1 = points[points.count - 1]
        context.addQuadCurve(to: CGPoint(x: p1.x, y: p1.y), control: CGPoint(x: p0.x, y: p0.y))
    }
}
