...
class SampleView : BaseView {
    ...
    override func setup() {
        ...
        tween(obj: ball, start: ball.x, target: 800, duration: 1, easingFunc: easeInOutQuad)
        {
            [unowned self](b, newValue) in
            b.x = newValue
            self.setNeedsDisplay()
        }
    }
    
    func tween<T>(obj:T, start:Float, target:Float, duration:Float, easingFunc:@escaping (Float, Float, Float, Float) -> Float, render:@escaping(T, Float) -> ()) {
        ...
        func updateAnim() {
            ...
            if time < duration {
                render(obj, easingFunc(time, start, change, duration))
